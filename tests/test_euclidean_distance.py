...
def test_euclidean_distance_calculates_properly():
    ...
    assert math.isclose(result, 1.13137085, rel_tol=1e-5), print(result)

    # Attempt at a standard vector
    result = euclidean_distance([0.2, 3.0, 1.4], [2.0, 2.7, 1.3])
    assert math.isclose(result, 1.827566688, rel_tol=1e-5), print(result)

    # all int vectors

    result = euclidean_distance([1, 2, 3], [3, 2, 4])

    assert math.isclose(result, 2.236067977, rel_tol=1e-5), print(result)
