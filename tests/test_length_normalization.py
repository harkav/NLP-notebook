import pytest
from utils.length_normalization import length_normalization
import math
...
def test_proper_calculation_length_normalization():
    ...
    assert math.isclose(result[0], 0.059654999, rel_tol=1e-2), print(result[0])
    assert math.isclose(result[1], 0.298274993, rel_tol=1e-2), print(result[1])
    assert math.isclose(result[2], 0.954479978, rel_tol=1e-2), print(result[2])
