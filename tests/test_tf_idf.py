...
def test_inverse_frequency_calculates_correctly():
    assert math.isclose(
        inverse_document_frequency("kvakk", test_docs), 0.287679572, rel_tol=1e-5
    )
    assert math.isclose(
        inverse_document_frequency("Erna", test_docs), 1.386294361, rel_tol=1e-5
    )
    assert math.isclose(
...
def test_tf_idf_calculates_correctly():
    assert math.isclose(
        tf_idf("Kvakk", test_docs[0], test_docs), 0.575359144, rel_tol=1e-5
    )
    assert math.isclose(tf_idf("Erna", test_docs[0], test_docs), 0, rel_tol=1e-5)
    assert math.isclose(
        tf_idf("Kvakk", "Kvakk", ["kvakk", "kvakk"]), -0.405465108 * 1, rel_tol=1e-5
    )
